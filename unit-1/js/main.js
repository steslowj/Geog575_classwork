...
/* Activity 4 built with Activity 3*/
...
function cities(){
	...
	document.querySelector("#mydiv").insertAdjacentElement('afterbegin', table);    //Inserts table to the div, before any other div children
...
//Activity 4 debugged code for ajax fetch

//Creates a function to use data from the fetch request, outside of the fetch request for data manip safety
function debugCallback(response){
	document.querySelector("#mydiv").insertAdjacentHTML('beforeend', '<br>GeoJSON data: </br>' + JSON.stringify(response));
    //JSON.stringify converts the parameter containing our fetched JSON data into a readible string
};

function debugAjax(){    //Creates a function to call the fetch. Included in initialize function at top of main.js
	
	fetch("data/MegaCities.geojson")  //fetch request with file location as parameter, "get" omitted and therefore implied
		.then(function(response){     //converts the fetched data into JSON format
            return response.json();
        })
        .then(debugCallback);         //passes JSON data to debugCallback function
